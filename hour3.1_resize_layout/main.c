/* pqview - PostgreSQL Query Client - Hour 3.1: Resize Layout */
...
void ResizeControls(HWND hwnd)
{
    ...
    /* Calculate debug group height (fills remaining space) */
    debugGroupHeight = clientHeight - margin - connectionHeight - margin -
                       connectButtonHeight - margin - queryGroupHeight - margin;

    /* Ensure minimum height for debug group */
    ...
    MoveWindow(g_hwndDebugGroup,
        margin, margin + connectionHeight + 10 + connectButtonHeight + 10 + queryGroupHeight + 10,
        groupWidth, debugGroupHeight,
        TRUE);

    /* Resize Debug output listbox (inside debug group) */
    MoveWindow(g_hwndDebug,
        margin + 10, margin + connectionHeight + 10 + connectButtonHeight + 10 + queryGroupHeight + 10 + 20,
