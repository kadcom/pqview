/* pqview - PostgreSQL Query Client - Hour 2.1: Modern UI (Windows 95 Style) */
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    wc.hbrBackground = (HBRUSH)(COLOR_BTNFACE + 1); /* Windows 95 dialog color */
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    switch (uMsg)
    {
        case WM_CREATE:
        {
            /* Create fonts - MS Sans Serif 8pt (Windows 95 standard) */
            g_hFont = CreateFont(
                ...
                "MS Shell Dlg"              /* Facename (Windows 95 default) */
            );
            ...
            g_hMonoFont = CreateFont(
                -13, 0, 0, 0,  /* Slightly larger for better readability */
                FW_NORMAL,
                FALSE, FALSE, FALSE,
                DEFAULT_CHARSET,
                OUT_DEFAULT_PRECIS,
                CLIP_DEFAULT_PRECIS,
                DEFAULT_QUALITY,
                FIXED_PITCH | FF_MODERN,    /* Fixed pitch for monospace */
                "Courier New"
            );
...
void CreateControls(HWND hwnd)
{
    ...
    SendMessage(g_hwndDebug, WM_SETFONT, (WPARAM)g_hMonoFont, TRUE);  /* Use monospace font */
}
